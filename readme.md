...
1. Provisioned a CCE cluster (managed K8s) on T Cloud Public (OpenTelekomCloud) using Terraform. The infrastructure code lives in the sibling [12-terraform-exercises repo](https://github.com/MonikaJassova/12-terraform-exercises/tree/prometheus) under `environments/prometheus/`.
...
To prove the two custom rules actually fire (and email), I forced each condition on a real node and watched the alert transition `inactive -> firing -> resolved`, checking both the Prometheus UI (`/alerts`) and the Alertmanager UI/API.
...
1. `KubernetesPodCrashLooping` (Pod cannot start / crash loop)
   - created a deliberately crash-looping pod. A plain `command: ["true"]` is throttled by CrashLoopBackoff (one restart per ~10s), which takes ~10 min to cross the `> 5` threshold. Instead I used a pod that runs fine but has an always-failing liveness probe, so the kubelet restarts it every ~5s and it crosses the threshold in ~30s:
     ```yaml
     containers:
     - name: crash-test
       image: busybox
...
- **Crash-loop rule is cumulative, not rate-based (known limitation).** `kube_pod_container_status_restarts_total > 5` counts restarts over the pod's *entire lifetime* (`for: 0m`), so once a pod ever crosses 5 restarts it stays `critical` until the pod is recreated (counter reset) — even if it has been healthy since. A rate-based variant such as `increase(kube_pod_container_status_restarts_total[10m]) > 5` would match "actively crash-looping" more precisely, at the cost of a shorter detection window. Kept as-is for simplicity.
- **Removed the misleading `namespace: monitoring` label from the custom rules.** `HostHighCpuLoad` (node-level) and `KubernetesPodCrashLooping` previously carried a static `namespace: monitoring` label. For a node alert, "namespace" is meaningless; for the pod rule it overrode the pod's real namespace (from `kube_pod_info`) with "monitoring". Removed so alerts carry their natural labels. Routing is by `alertname`, so delivery is unchanged.
...
1. Configured alert rules for Redis down and Redis has too many connections and applied them for Prometheus Operator to pick up: `kubectl apply -f redis-rules.yaml`
   ...
   - tested `RedisDown` for real: `kubectl scale deployment redis-cart --replicas=0` -> rule flips to `firing` (it has `for: 0m`) -> restore with `--replicas=2` -> back to `inactive` and `redis_up = 1` again (full fire/resolve cycle confirmed)
...
1. Created a dashboard for metrics of our Node.js app with Requests per second visualization (`rate(http_request_operations_total[2m])`) and Request duration (`rate(http_request_duration_seconds_sum[2m])`)
